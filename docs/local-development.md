...
* **Bootstrap**: the checked-in CSS and JavaScript provide responsive layout,
  ...
  variables.
* **Chart.js**: the dashboard's `wwwroot/charts.js` creates the trend chart
  through JavaScript interop and recolors its axes, grid, legend, and tooltip
...
5. A sync obtains a valid access token, requests the configured date range,
   merges `(UserKey, MetricDate)` rows, and records sync history.
